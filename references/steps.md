...
CTF flag on top of takeover? One sweep of the DC, not a dozen `atexec` calls:
`revshell.sh flags -u Administrator -H {{HASH}} -d {{DOMAIN}}` (DC from `$ADTK_DC`,
`revshell.md`). The takeover proof is still DCSync, not the flag.
